"""JST wordmark with colored .jl suffix"""
function JSTWordmark()
    NavLink("./",
        RawHtml("""JavaScriptTarget<span style="color:var(--jl-dot)">.</span><span style="color:var(--jl-j)">j</span><span style="color:var(--jl-l)">l</span>""");
        class = "text-lg font-serif font-bold text-warm-900 dark:text-warm-100 hover:opacity-80 transition-opacity no-underline",
        active_class = ""
    )
end
